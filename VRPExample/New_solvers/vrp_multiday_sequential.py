...
# Configure logging with a new formatter that includes the log level
logging.basicConfig(
    level=logging.INFO,
...
# Set default log level to WARNING to suppress most logs
logger.setLevel(logging.WARNING)

# Create a special route logger for concise vehicle stop sequences
route_logger = logging.getLogger('route_logger')
# Keep route logger at INFO level
...
class SequentialMultiDayVRP:
    ...
    def solve_single_day(self, day_num, day_locations, active_vehicles, force_return_to_depot=False):
        ...
        if force_return_to_depot:
            route_logger.info("  🏠 FINAL DAY: All vehicles must return to depot")
        
        if not active_vehicles:
            route_logger.warning("  ⚠️ No active vehicles for this day")
            return None
        ...
        for vehicle_idx, vehicle in enumerate(active_vehicles):
            ...
            if not vehicle_available_customers:
                ...
                # Check if we need to force return to depot (final day logic)
                if force_return_to_depot and current_pos != 0:  # Not at depot and must return
                    print(f"    🏠 FINAL DAY: No customers to visit, but must return to depot from overnight position")
                    # Calculate return journey to depot
                    try:
                    ...
                    # Force return to depot even if it exceeds time limit (final day)
                    ...
                    print(f"    {current_time:05.1f} - Return to depot completed (FINAL DAY - no customers remaining)")
                elif current_pos != 0:  # Not at depot and not final day
                    print(f"    Staying at current position (no customers to visit)")
                    # Add current position as final stop
                    route_stops.append({
                        'location_id': start_location['id'],
                        'coordinates': (start_location['x'], start_location['y']),
                        'is_overnight': False,
                    ...
                    # Vehicle stays at depot - no need to add return trip
                
                # Skip all the return-to-depot logic since vehicle has no work to do
                day_routes[vehicle_id] = {
                    'stops': route_stops,
                    'distance': route_distance,
                    'total_demand': total_demand,
                    'ends_with_overnight': False,  # No overnight needed if no work
                ...
                while vehicle_available_customers and vehicle_state['remaining_capacity'] > 0:
                    ...
                    for customer_rel_idx in available_indices:
                        ...
                        if projected_time_after_customer > self.daily_time_limit_minutes:
                            # Can't reach the customer - check if vehicle should stay at depot instead
                            if current_pos == 0 and len(route_stops) == 1 and current_time == 0:
                                # Vehicle is at depot with no work done - don't create overnight stop
                                print(f"      🏠 CAN'T REACH CUSTOMER but vehicle is idle at depot - will stay at depot")
                                customer_assigned = False  # Don't assign, try next customer
                                continue  # Try next customer instead of creating overnight stop
                            ...
                            if remaining_time > 30:  # Only move if we have meaningful time left
                                ...
                                # Use OSRM-based overnight placement instead of straight line
                                try:
                                    overnight_id, overnight_coords = self.create_overnight_stop_on_osrm_route(
                                        vehicle_id, current_coords, (customer['x'], customer['y']),
                                        max_travel_time, travel_speed_kmh=60, day_num=day_num
                                    )
                                    overnight_x, overnight_y = overnight_coords
                                    print(f"         Moving {travel_ratio:.1%} of the way towards {customer['id']} via OSRM route")
                                except Exception as e:
                                    logger.warning(f"OSRM overnight placement failed: {e}, using straight line fallback")
                                    ...
                                    print(f"         Moving {travel_ratio:.1%} of the way towards {customer['id']} via straight line (fallback)")
                        ...
                        if remaining_time > 30:  # Only move if we have meaningful time left
                            ...
                            # Calculate intermediate position on the road to customer using OSRM
                            try:
                                overnight_id, overnight_coords = self.create_overnight_stop_on_osrm_route(
                                    vehicle_id, current_coords, (customer['x'], customer['y']),
                                    max_travel_time, travel_speed_kmh=60, day_num=day_num
            ...
            if vehicle_id not in overnight_locations:
                # Check if vehicle has no deliveries to make (no customers visited beyond starting position)
                # This means the vehicle should stay where it is instead of making unnecessary trips
                has_deliveries = len(route_stops) > 1 and any(stop['demand'] > 0 for stop in route_stops)
                
                if not has_deliveries:
                    print(f"      🏠 {vehicle_id} has no deliveries - staying at current position")
                    # Vehicle has no work to do - stays where it is (depot or overnight position)
                    # No need for return trip or overnight logic
                    pass  # Vehicle already has its starting position in route_stops
                else:
                    try:
                        return_time = self.distance_calculator.time_matrix[current_pos][0] if current_pos < len(self.locations) else 30
                        return_distance = self.distance_calculator.distance_matrix[current_pos][0] if current_pos < len(self.locations) else 20
                    ...
                    if force_return_to_depot:
                        ...
                        if remaining_time > 30:  # Only move if we have meaningful time left
                            ...
                            # Calculate intermediate position on the road to depot using OSRM
                            try:
                                overnight_id, overnight_coords = self.create_overnight_stop_on_osrm_route(
                                    vehicle_id, current_coords, depot_coords,
                                    max_travel_time, travel_speed_kmh=60, day_num=day_num
                                )
                                overnight_x, overnight_y = overnight_coords
                                print(f"         Moving {travel_ratio:.1%} of the way towards depot via OSRM route")
                            except Exception as e:
                                logger.warning(f"OSRM overnight placement failed: {e}, using straight line fallback")
                                # Fallback to straight-line calculation
                                ...
                                overnight_x = current_coords[0] + (dx * travel_ratio)
                                overnight_y = current_coords[1] + (dy * travel_ratio)
                                print(f"         Moving {travel_ratio:.1%} of the way towards depot via straight line (fallback)")
    ...
    def solve_sequential_multiday(self, max_days=3):
        ...
        for day_num in range(1, max_days + 1):
            ...
            # Log the day solution
            self.log_day_solution(day_num, day_solution)
            
            # Update state for next day
            visited_today = day_solution['visited_customers']
            overnight_vehicles = day_solution['overnight_locations']
            
            # Log the overnight status
            if overnight_vehicles:
                route_logger.info(f"👉 Day {day_num}: {len(overnight_vehicles)} vehicles need overnight stops")
                for v_id, overnight_loc in overnight_vehicles.items():
                    # Check if overnight_loc is a dictionary or other data type
                    if isinstance(overnight_loc, dict):
                        is_road = overnight_loc.get('is_road_position', False)
                        loc_id = overnight_loc.get('id', 'unknown')
                        position_type = "road position" if is_road else "depot/customer location"
                        
                        # Get more descriptive location information
                        orig_index = overnight_loc.get('original_customer_idx')
                        
                        # Try to get location ID from original index if needed
                        if loc_id == 'unknown' and orig_index is not None and orig_index < len(self.locations):
                            loc_id = self.locations[orig_index].get('id', f'loc_{orig_index}')
                        
                        # Get coordinates for clearer identification
                        x_coord = overnight_loc.get('x', None)
                        y_coord = overnight_loc.get('y', None)
                        
                        # If coordinates are missing, try to get them from original index
                        if (x_coord is None or y_coord is None) and orig_index is not None and orig_index < len(self.locations):
                            x_coord = self.locations[orig_index].get('x', '?')
                            y_coord = self.locations[orig_index].get('y', '?')
                        
                        # Format coordinates for display
                        coords = ""
                        if isinstance(x_coord, (int, float)) and isinstance(y_coord, (int, float)):
                            coords = f" ({x_coord:.2f}, {y_coord:.2f})"
                        
                        # Enhanced overnight position logging
                        if loc_id == 'unknown' or loc_id.startswith('road_overnight'):
                            target = overnight_loc.get('target_customer', 'unknown')
                            if target != 'unknown':
                                route_logger.info(f"   🛌 Vehicle {v_id} stopping at {loc_id}{coords} - {position_type} heading to {target}")
                            else:
                                route_logger.info(f"   🛌 Vehicle {v_id} stopping at {loc_id}{coords} - {position_type}")
                        else:
                            route_logger.info(f"   🛌 Vehicle {v_id} stopping at {loc_id}{coords} - {position_type}")
                    else:
                        # Handle case where overnight_loc is not a dictionary
                        loc_id = str(overnight_loc)
                        position_type = "integer index, check locations array"
                        route_logger.info(f"   🛌 Vehicle {v_id} stopping at {loc_id} ({position_type})")
                    ...
                    if visited_id == 'cormano_mi':
                        ...
                        for idx, loc in enumerate(self.locations):
                            if loc['id'] == 'depot_bay_1':
                                ...
                                route_logger.info(f"    📦 Consumed depot_bay_1 after visiting {visited_id}")
                                break
                        ...
                        for idx, loc in enumerate(self.locations):
                            if loc['id'] == 'depot_bay_2':
                                depot_bays_to_remove.add(idx)
                                route_logger.info(f"    📦 Consumed depot_bay_2 after visiting {visited_id}")
                                break
                        ...
                        if bay_number == '1':
                            ...
                            for idx, loc in enumerate(self.locations):
                                if loc['id'] == 'cormano_mi':
                                    ...
                                    route_logger.info(f"    📦 Picked up cargo from {visited_id} for cormano_mi delivery")
                                    break
                        elif bay_number == '2':
                            ...
                            for idx, loc in enumerate(self.locations):
                                if loc['id'] == 'malmo_sweden':
                                    depot_bays_to_remove.add(visited_idx)  # Remove the bay itself
                                    route_logger.info(f"    📦 Picked up cargo from {visited_id} for malmo_sweden delivery")
                                    break
            ...
            if is_final_day:
                route_logger.info(f"  🏠 FINAL DAY: Ensuring all vehicles return to depot")
                for vehicle in active_vehicles:
                    ...
                    route_logger.info(f"    🏠 {vehicle_id} returned to depot (final day complete)")
            else:
                ...
                for vehicle_id, overnight_location_idx in overnight_vehicles.items():
                    ...
                    location_info = ""
                    if isinstance(overnight_location_idx, dict):
                        # It's a location dictionary
                        loc_id = overnight_location_idx.get('id', 'unknown')
                        if loc_id != 'unknown':
                            location_info = f"{loc_id}"
                        
                        # Try to get from original index
                        orig_idx = overnight_location_idx.get('original_customer_idx')
                        if orig_idx is not None and orig_idx < len(self.locations):
                            orig_loc_id = self.locations[orig_idx].get('id', f"idx_{orig_idx}")
                            if location_info:
                                location_info += f" ({orig_loc_id})"
                            else:
                                location_info = f"{orig_loc_id}"
                        
                        # Add coordinates if available
                        x = overnight_location_idx.get('x')
                        y = overnight_location_idx.get('y')
                        if isinstance(x, (int, float)) and isinstance(y, (int, float)):
                            location_info += f" at ({x:.2f}, {y:.2f})"
                    elif isinstance(overnight_location_idx, int):
                        # It's just an index
                        if overnight_location_idx < len(self.locations):
                            loc = self.locations[overnight_location_idx]
                            loc_id = loc.get('id', f"idx_{overnight_location_idx}")
                            location_info = f"{loc_id}"
                            # Add coordinates if available
                            x = loc.get('x')
                            y = loc.get('y')
                            ...
                            location_info = f"idx_{overnight_location_idx}"
                    
                    if not location_info:
                        location_info = str(overnight_location_idx)
                    
                    route_logger.info(f"    📍 {vehicle_id} will start Day {day_num + 1} at {location_info}")
                ...
                for vehicle in active_vehicles:
                    ...
                    if vehicle_id not in overnight_vehicles:
                        ...
                        self.vehicle_states[vehicle_id]['current_location_idx'] = 0  # Return to depot
                        self.vehicle_states[vehicle_id]['overnight_position'] = None
                        route_logger.info(f"    🏠 {vehicle_id} returned to depot (inactive for remaining days)")
    ...
    def log_vehicle_stop_sequence(self, day_num: int, vehicle_id: str, route_stops: list):
        """
        Generate a concise, structured log of a vehicle's stop sequence for debugging.
        
        Args:
            day_num: The day number
            vehicle_id: The vehicle ID
            route_stops: List of stop dictionaries with location details
        """
        stop_types = {
            'depot': '🏭',
            'customer': '🏢',
        ...
        # Format header for this vehicle's route
        header = f"DAY {day_num} | VEHICLE {vehicle_id} | ROUTE SEQUENCE:"
        route_logger.info(f"{'='*80}")
        route_logger.info(header)
        
        # Track if we have overnight stops
        has_overnight = False
        total_distance = 0
        total_time = 0
        
        # Format each stop in the route
        for i, stop in enumerate(route_stops):
            # Get proper location ID
            original_index = stop.get('original_customer_idx')
            location_id = stop.get('id', None) 
            
            if location_id is None and original_index is not None and original_index < len(self.locations):
                # Try to find the ID from the original index
                location_id = self.locations[original_index].get('id', f'loc_{original_index}')
            elif location_id is None:
                location_id = stop.get('location_id', f'stop_{i}')
            
            if location_id == 'unknown':
                location_id = f"loc_{original_index}" if original_index is not None else f"stop_{i}"
            
            # Determine location type
            location_type = 'depot' if location_id == 'depot' else (
                           'overnight' if stop.get('is_overnight_node', False) and not stop.get('is_road_position', False) else (
                           'road_overnight' if stop.get('is_road_position', True) else 'customer'))
            
            icon = stop_types.get(location_type, '📍')
            
            # Get proper coordinates
            x_coord = stop.get('x', None)
            y_coord = stop.get('y', None)
            
            # If coordinates are missing, try to get them from the locations array using original_customer_idx
            if (x_coord is None or y_coord is None or x_coord == '?' or y_coord == '?') and original_index is not None and original_index < len(self.locations):
                x_coord = self.locations[original_index].get('x', '?')
                y_coord = self.locations[original_index].get('y', '?')
            ...
            # Get travel details if available
            distance = stop.get('distance_from_prev', 0)
            time = stop.get('time_from_prev', 0)
            total_distance += distance
            total_time += time
            
            # Mark overnight stops
            if location_type in ('overnight', 'road_overnight'):
                has_overnight = True
                
            # Format arrival time if available
            arrival_time = ""
            if 'arrival_time' in stop:
                hours = int(stop['arrival_time'] // 60)
                minutes = int(stop['arrival_time'] % 60)
                arrival_time = f"{hours:02d}:{minutes:02d}"
                
            # Build the stop description
            if i == 0:
                description = f"{icon} START: {location_id} {coords}"
            else:
                prev_distance = f"{distance:.2f}km" if distance > 0 else "0km"
                prev_time = f"{time:.0f}min" if time > 0 else "0min"
                description = f"{icon} STOP {i}: {location_id} {coords} | From prev: {prev_distance} ({prev_time})"
                if arrival_time:
                    description += f" | Arrival: {arrival_time}"
            
            # Add any special notes
            notes = []
            if stop.get('demand', 0) > 0:
                notes.append(f"Demand: {stop.get('demand')}")
            if stop.get('service_time', 0) > 0:
                notes.append(f"Service: {stop.get('service_time')}min")
            if stop.get('is_road_position', False):
                notes.append(f"Road position to {stop.get('target_customer', '?')}")
                
            if notes:
                description += f" | {', '.join(notes)}"
                
            route_logger.info(description)
            
        # Summary line
        summary = f"SUMMARY: {len(route_stops)} stops"
        summary += f" | Total distance: {total_distance:.2f}km"
        summary += f" | Total time: {total_time:.0f}min"
        if has_overnight:
            summary += " | ⚠️ HAS OVERNIGHT STOPS"
        route_logger.info(summary)
        route_logger.info(f"{'='*80}")
    
    def log_day_solution(self, day_num, day_solution):
        """
        Log a comprehensive, structured summary of a day's solution showing stop sequences.
        
        Args:
            day_num: The day number
            day_solution: The solution for this day
        """
        route_logger.info(f"\n{'#'*80}")
        route_logger.info(f"DAY {day_num} - SOLUTION SUMMARY")
        route_logger.info(f"{'#'*80}")
        
        visited_customers = day_solution.get('visited_customers', set())
        total_customers = len(visited_customers)
        total_distance = day_solution.get('total_distance', 0)
        total_time = day_solution.get('total_time', 0)
        
        # Main statistics
        route_logger.info(f"Customers visited: {total_customers}")
        route_logger.info(f"Total distance: {total_distance:.2f} km")
        route_logger.info(f"Total time: {total_time:.0f} minutes")
        route_logger.info(f"Overnight stops: {len(day_solution.get('overnight_locations', {}))}")
        
        # Log each route
        routes = day_solution.get('routes', {})
        for vehicle_id, route_data in routes.items():
            route_stops = route_data.get('stops', [])
            
            # Use our route logger method
            self.log_vehicle_stop_sequence(day_num, vehicle_id, route_stops)
            
        route_logger.info(f"{'#'*80}\n")
    
    def interpolate_osrm_route(self, start_coords: Tuple[float, float], 
        ...
        try:
            ...
            osrm_url = getattr(self.distance_calculator, 'osrm_url', "http://router.project-osrm.org")
            ...
            if data['code'] != 'Ok' or not data['routes']:
                logger.warning(f"OSRM route failed: {data.get('code', 'Unknown error')}")
                return self.interpolate_straight_line(start_coords, end_coords, distance_ratio)
            
            # Get route coordinates
            route_coords = data['routes'][0]['geometry']['coordinates']
            
            if len(route_coords) < 2:
                logger.warning(f"OSRM returned insufficient coordinates")
                return self.interpolate_straight_line(start_coords, end_coords, distance_ratio)
            
            # Calculate cumulative distances along the route using Haversine formula
            ...
            for i in range(1, len(route_coords)):
                ...
                segment_distance = self.haversine_distance(prev_coord[1], prev_coord[0], 
            ...
            logger.warning(f"Error interpolating OSRM route: {e}")
            return self.interpolate_straight_line(start_coords, end_coords, distance_ratio)
    
    def interpolate_straight_line(self, start_coords: Tuple[float, float], 
                                end_coords: Tuple[float, float], 
                                distance_ratio: float) -> Tuple[float, float]:
    ...
    def create_overnight_stop_on_osrm_route(self, vehicle_id: str, 
                                           ...
                                           day_num: int) -> Tuple[str, Tuple[float, float]]:
        """
        ...
        try:
            ...
            try:
                ...
                if (current_idx is not None and target_idx is not None and 
                    ...
                    # Fallback to direct OSRM call - this should rarely happen
                    total_route_distance = self.get_osrm_distance_direct(current_position, target_position)
                
                # Calculate how far along the route we can travel
                if total_route_distance > 0:
                    distance_ratio = min(remaining_distance_km / total_route_distance, 0.8)  # Max 80%
                else:
                    distance_ratio = 0.5  # Default fallback
                
                # Get overnight position on OSRM route
                overnight_coords = self.interpolate_osrm_route(
                ...
                overnight_id = f"osrm_overnight_{vehicle_id}_day{day_num}"
                
                logger.info(f"🛏️ Created OSRM-based overnight stop for {vehicle_id}: {overnight_coords} "
                           f"({distance_ratio:.1%} along route, {remaining_distance_km:.1f}km from start)")
                
                return overnight_id, overnight_coords
                
            except Exception as e:
                logger.warning(f"OSRM distance lookup failed: {e}, using straight line fallback")
                return self.create_overnight_stop_straight_line(vehicle_id, current_position, 
                                                               target_position, remaining_time, 
                                                               travel_speed_kmh, day_num)
                
        except Exception as e:
            logger.warning(f"OSRM overnight placement failed: {e}, using straight line")
            return self.create_overnight_stop_straight_line(vehicle_id, current_position, 
                                                           target_position, remaining_time, 
                                                           travel_speed_kmh, day_num)
    
    def create_overnight_stop_straight_line(self, vehicle_id: str,
                                          current_position: Tuple[float, float],
                                          target_position: Tuple[float, float],
                                          remaining_time: float,
                                          travel_speed_kmh: float,
                                          day_num: int) -> Tuple[str, Tuple[float, float]]:
        """Fallback method for straight-line overnight placement."""
        # Calculate how far we can travel in straight line
        remaining_distance_km = (remaining_time / 60.0) * travel_speed_kmh
        ...
        # Calculate ratio (max 80% of the way)
        travel_ratio = min(remaining_distance_km / straight_distance, 0.8) if straight_distance > 0 else 0.5
        
        # Calculate overnight position
        overnight_coords = self.interpolate_straight_line(current_position, target_position, travel_ratio)
        
        overnight_id = f"road_overnight_{vehicle_id}_day{day_num}"
        
        logger.info(f"🛏️ Created straight-line overnight stop for {vehicle_id}: {overnight_coords} "
                   f"({travel_ratio:.1%} along straight line)")
        
        return overnight_id, overnight_coords
    
    def get_location_index(self, position: Tuple[float, float]) -> Optional[int]:
        """Find the index of a position in the locations list."""
        for i, location in enumerate(self.locations):
            if (abs(location['x'] - position[0]) < 0.001 and 
                abs(location['y'] - position[1]) < 0.001):
                return i
        return None
    
    def get_osrm_distance_direct(self, pos1: Tuple[float, float], pos2: Tuple[float, float]) -> float:
        """Get OSRM distance directly via API call."""
        try:
            import requests
            osrm_url = getattr(self.distance_calculator, 'osrm_url', "http://router.project-osrm.org")
            url = f"{osrm_url}/route/v1/driving/{pos1[0]},{pos1[1]};{pos2[0]},{pos2[1]}"
            ...
            if data['code'] == 'Ok' and data['routes']:
                ...
                return self.haversine_distance(pos1[1], pos1[0], pos2[1], pos2[0])
                
        except Exception as e:
            logger.warning(f"Direct OSRM call failed: {e}")
            return self.haversine_distance(pos1[1], pos1[0], pos2[1], pos2[0])

    def recalculate_vehicle_distances(self, solution):
        """
        Recalculate vehicle distances by filtering out overnight nodes and using real OSRM routes.
        
        The issue is that overnight nodes are dynamically created during optimization but aren't in the 
        OSRM database, so distance calculations using them are incorrect. This method:
        1. Gets each vehicle's full route
        2. Filters out overnight nodes 
        3. Uses real A->B routes from the database to calculate accurate distances
        """
        print("🔧 Recalculating vehicle distances using real OSRM routes...")
        
        corrected_solution = solution.copy()
        total_corrected_distance = 0
        
        for vehicle_id, route_data in solution.get('vehicle_routes', {}).items():
            print(f"  📊 Processing {vehicle_id}...")
            
            # Debug: Print route structure
            print(f"    🔍 Route data keys: {list(route_data.keys())}")
            
            # Get the full route for this vehicle
            full_route = route_data.get('full_route', [])
            print(f"    📍 Full route has {len(full_route)} items")
            
            # Debug: Show first few items in route
            if len(full_route) > 0:
                print(f"    🔍 First 3 route items: {full_route[:3]}")
            
            # Filter out overnight nodes and day markers - keep only real locations
            real_stops = []
            for stop in full_route:
                if isinstance(stop, dict):
                    # Skip day markers
                    if stop.get('is_day_marker', False):
                        continue
                    # Skip overnight stops
                    if stop.get('is_overnight', False):
                        continue
                    # Keep real customer locations and depot
                    location_id = stop.get('location_id')
                    if location_id and location_id != f'=== DAY {stop.get("day", 1)} ===':
                        # Additional check: make sure this isn't an overnight node
                        if 'overnight' not in str(location_id).lower() and 'road_overnight' not in str(location_id).lower():
                            # Convert bay locations to depot for distance calculation
                            if 'bay' in str(location_id).lower() or location_id == 'depot':
                                real_stops.append('depot')
                            else:
                                real_stops.append(location_id)
                elif isinstance(stop, str):
                    # Handle string format stops
                    if ('day' not in stop.lower() and 
                        'overnight' not in stop.lower() and 
                        'road_overnight' not in stop.lower() and
                        stop != '=== DAY'):
                        # Convert bay locations to depot for distance calculation
                        if 'bay' in stop.lower() or stop == 'depot':
                            real_stops.append('depot')
                        else:
                            real_stops.append(stop)
            
            print(f"    🗺️ Found {len(real_stops)} real stops (filtered out overnight nodes)")
            
            # Remove consecutive depot visits (bay-to-bay movements at depot = 0 distance)
            optimized_stops = []
            for i, stop in enumerate(real_stops):
                if i == 0 or stop != real_stops[i-1]:
                    optimized_stops.append(stop)
            
            print(f"    🚀 Optimized to {len(optimized_stops)} stops (removed consecutive depot visits)")
            
            # Calculate distance using real OSRM routes between consecutive stops
            corrected_distance = 0
            
            if len(optimized_stops) >= 2:
                for i in range(len(optimized_stops) - 1):
                    from_location = optimized_stops[i]
                    to_location = optimized_stops[i + 1]
                    
                    # Try to get the distance from the distance calculator
                    try:
                        # The distance calculator expects location indices
                        from_idx = None
                        to_idx = None
                        
                        # Find the location indices
                        for idx, loc in enumerate(self.locations):
                            if loc['id'] == str(from_location):
                                from_idx = idx
                            if loc['id'] == str(to_location):
                                to_idx = idx
                        
                        if from_idx is not None and to_idx is not None:
                            # Use the distance matrix which already contains OSRM-calculated distances
                            if (hasattr(self, 'distance_calculator') and 
                                hasattr(self.distance_calculator, 'distance_matrix') and
                                from_idx < len(self.distance_calculator.distance_matrix) and
                                to_idx < len(self.distance_calculator.distance_matrix[from_idx])):
                                
                                distance_km = self.distance_calculator.distance_matrix[from_idx][to_idx]
                                corrected_distance += distance_km
                            else:
                                # Fallback to direct coordinate calculation
                                from_lat = self.locations[from_idx].get('lat', self.locations[from_idx].get('y', 0))
                                from_lon = self.locations[from_idx].get('lon', self.locations[from_idx].get('x', 0))
                                to_lat = self.locations[to_idx].get('lat', self.locations[to_idx].get('y', 0))
                                to_lon = self.locations[to_idx].get('lon', self.locations[to_idx].get('x', 0))
                                
                                distance_km = self.haversine_distance(from_lat, from_lon, to_lat, to_lon)
                                corrected_distance += distance_km
                        else:
                            # Location not found, estimate distance
                            print(f"    ⚠️ Location not found: {from_location} or {to_location}")
                            corrected_distance += 50  # Default fallback distance
                                
                    except Exception as e:
                        print(f"    ⚠️ Error calculating distance {from_location} -> {to_location}: {e}")
                        # Fallback to haversine distance
                        try:
                            from_loc = next(loc for loc in self.locations if loc['id'] == str(from_location))
                            to_loc = next(loc for loc in self.locations if loc['id'] == str(to_location))
                            
                            # Get coordinates with fallback
                            from_lat = from_loc.get('lat', from_loc.get('y', 0))
                            from_lon = from_loc.get('lon', from_loc.get('x', 0))
                            to_lat = to_loc.get('lat', to_loc.get('y', 0))
                            to_lon = to_loc.get('lon', to_loc.get('x', 0))
                            
                            distance = self.haversine_distance(from_lat, from_lon, to_lat, to_lon)
                            corrected_distance += distance
                        except Exception as fallback_error:
                            print(f"    ❌ Could not calculate distance for {from_location} -> {to_location}: {fallback_error}")
                            continue
            
            # Update the solution with corrected distance
            old_distance = route_data.get('total_distance', 0)
            corrected_solution['vehicle_routes'][vehicle_id]['total_distance'] = corrected_distance
            total_corrected_distance += corrected_distance
            
            print(f"    📏 Distance: {old_distance:.1f}km -> {corrected_distance:.1f}km (Δ: {corrected_distance - old_distance:+.1f}km)")
        
        print(f"  ✅ Total corrected distance: {total_corrected_distance:.1f}km")
        print(f"  📊 Distance correction completed!")
