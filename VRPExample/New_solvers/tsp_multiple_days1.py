...
def solve_with_scenario(days=3, start=6, end=18, waittime=30, timelimit=10, debug=False, guided_local_off=False, skip_mornings=False):
    """Solve TSP1 with specified parameters"""
    
    # Configure parameters similar to main()
    day_start = start * 3600
    day_end = end * 3600
    
    if days <= 0:
        raise ValueError("--days parameter must be 1 or more")
    
    num_days = days - 1
    node_service_time = waittime * 60
    ...
    num_nodes = T.num_nodes()
    night_nodes = range(num_nodes, num_nodes+num_days)
    morning_nodes = range(num_nodes+num_days, num_nodes+num_days+num_days) if not skip_mornings else []
    total_nodes = num_nodes + len(night_nodes) + len(morning_nodes)
    ...
    # Register callbacks
    transit_callback_fn = partial(T.transit_callback, manager, day_end, night_nodes, morning_nodes)
    transit_callback_index = routing.RegisterTransitCallback(transit_callback_fn)
    routing.SetArcCostEvaluatorOfAllVehicles(transit_callback_index)
    
    time_callback_fn = partial(T.time_callback, manager, node_service_time, overnight_time, night_nodes, morning_nodes)
    time_callback_index = routing.RegisterTransitCallback(time_callback_fn)
    ...
    # Add count dimension for ordering constraints
    count_callback_fn = partial(T.count_callback, manager, night_nodes, morning_nodes)
    count_callback_index = routing.RegisterTransitCallback(count_callback_fn)
    routing.AddDimension(
    ...
    # Add disjunctions for customer nodes
    for cnode in range(num_nodes):
        routing.AddDisjunction([manager.NodeToIndex(cnode)], disjunction_penalty)
    
    # Set solver parameters
    search_parameters = pywrapcp.DefaultRoutingSearchParameters()
    search_parameters.first_solution_strategy = (routing_enums_pb2.FirstSolutionStrategy.PARALLEL_CHEAPEST_INSERTION)
    if not guided_local_off:
        search_parameters.local_search_metaheuristic = (routing_enums_pb2.LocalSearchMetaheuristic.GUIDED_LOCAL_SEARCH)
    ...
    while not routing.IsEnd(index):
        ...
        location_info = {
            'node': node,
            'min_time': solution.Min(cumultime),
            'max_time': solution.Max(cumultime)
        }
        
        # Add coordinate information if available
        coords = T.get_node_coordinates(node) if hasattr(T, 'get_node_coordinates') else None
        if coords:
            location_info['coordinates'] = coords
        
        result['Scheduled'].append(location_info)
        scheduled_route.append(location_info)
        
        # Add to total distance (rough calculation)
        if hasattr(T, 'distance_between_nodes'):
            next_index = solution.Value(routing.NextVar(index))
            if not routing.IsEnd(next_index):
                next_node = manager.IndexToNode(next_index)
                total_distance += T.distance_between_nodes(node, next_node) / 1000  # Convert to km
    ...
    return {
        ...
        'result': result,
        'scheduled_locations': len(result['Scheduled']),
        'dropped_locations': len(result['Dropped'])
    }
