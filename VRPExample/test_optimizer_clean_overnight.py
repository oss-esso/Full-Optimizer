...
# Main logger - set to WARNING to suppress most verbose logs
...
def create_interactive_vrp_map(scenario, solution, sequential_vrp):
    ...
    def get_cached_route_geometry(start_coords, end_coords):
        """Get cached route geometry from the sequential VRP's route database, or fetch from OSRM if missing."""
        try:
            # Access the cached route database from sequential_vrp
        ...
        if not daily_routes:
            ...
            if full_route:
                ...
                if isinstance(full_route, dict):
                    ...
                    print(f"  🗺️ Drawing cached OSRM routes for {vehicle_id} (flat route with {len(full_route)} stops)")
                    ...
                    if len(route_coords) > 1:
                        ...
                        for i in range(len(route_coords) - 1):
                            ...
                            # Get street-following route from cached database
                            street_route = get_cached_route_geometry(start_coord, end_coord)
                        ...
                        # Draw the complete route with cached OSRM-based paths
                        if len(all_route_coords) > 1:
                            folium.PolyLine(
                                ...
                                weight=4,
                                opacity=0.8,
                                popup=f"{vehicle_id} - Multi-Day Route (Cached OSRM)"
                            ).add_to(m)
                            print(f"    ✅ Added cached OSRM route with {len(all_route_coords)} points")
                        else:
                            # Fallback to simple line if cache is empty
                            folium.PolyLine(
        ...
        for day_num, day_route in daily_routes.items():
            ...
            if day_route.get('overnight_location') or day_route.get('overnight_position'):
                ...
                if overnight_pos:
                    ...
                    markers_added += 1
                    route_info[vehicle_id]['overnight_stays'] += 1
                    logger.debug(f"    🛏️ Added overnight marker at ({lat:.4f}, {lon:.4f})")
            
            # Add route lines for this day with cached OSRM-based routing
            if len(day_stops) > 1:
                route_coords = []
                ...
                # Draw cached OSRM-based route lines between consecutive stops
                if len(route_coords) > 1:
                    print(f"  🗺️ Drawing cached OSRM routes for {vehicle_id} - Day {day_num} ({len(route_coords)} stops)")
                    
                    # Get cached OSRM routes between consecutive stops
                    all_route_coords = []
                    ...
                    if len(all_route_coords) > 1:
                        folium.PolyLine(
                            ...
                            weight=4,
                            opacity=0.8,
                            popup=f"{vehicle_id} - Day {day_num} (Cached OSRM Routes)"
                        ).add_to(m)
                        print(f"    ✅ Added cached OSRM route with {len(all_route_coords)} points")
                    else:
                        # Fallback to simple line if cache is empty
    ...
    <div style="font-size: 11px;">
        ...
        using cached OSRM street routing<br>
    </div>
    </div>
    '''
